 ...
 *  </ol>
 */
trait ParadiseParsers extends NscScanners with ParadiseMarkupParsers with NscParsersCommon {
self =>
  ...
  class ParadiseSourceFileParser(val source: SourceFile) extends ParadiseParser {
    ...
    /** the markup parser */
    lazy val xmlp = new ParadiseMarkupParser(this, preserveWS = true)

    object symbXMLBuilder extends ParadiseSymbolicXMLBuilder(this, preserveWS = true) { // DEBUG choices
      val global: self.global.type = self.global
      def freshName(prefix: String): Name = ParadiseSourceFileParser.this.freshName(prefix)
    }
  ...
  class ParadiseOutlineParser(source: SourceFile) extends ParadiseSourceFileParser(source) {
  ...
  class ParadiseUnitParser(val unit: global.CompilationUnit, patches: List[BracePatch]) extends ParadiseSourceFileParser(unit.source) {
    ...
    def smartParse(): Tree = withSmartParsing {
      ...
      if (syntaxErrors.isEmpty) firstTry
        ...
        case patches  => new ParadiseUnitParser(unit, patches).parse()
      }
    }
  }
